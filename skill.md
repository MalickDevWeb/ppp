...
## 📏 RÈGLE ABSOLUE — LIMITE DE 100 LIGNES PAR FICHIER

> **Cette règle est non négociable. Aucun fichier généré ne peut dépasser 100 lignes.**

### Pourquoi 100 lignes maximum ?

| Raison | Explication |
|---|---|
| **Lisibilité** | Un fichier de 100 lignes se lit en 2 minutes. 500 lignes = fatigue cognitive |
| **Responsabilité unique** | Si tu as besoin de plus de 100 lignes, le fichier fait trop de choses |
| **Testabilité** | Un fichier court est une fonction/classe à responsabilité unique = testable en isolation |
| **Revue de code** | Un PR avec des fichiers courts est reviewé en minutes, pas en heures |
| **Refactoring** | Découper maintenant évite la dette technique future |

### Limites par type de fichier

| Type de fichier | Limite stricte | Action si dépassé |
|---|---|---|
| Composant React (`.tsx`) | **80 lignes** | Extraire un sous-composant |
| Hook React (`.ts`) | **80 lignes** | Extraire un sous-hook |
| Use Case (`.ts`) | **50 lignes** | Décomposer en sous-fonctions |
| Entité Domaine (`.ts`) | **60 lignes** | Extraire les fonctions dans un fichier `ProduitStock.helpers.ts` |
| Adaptateur (`.ts`) | **80 lignes** | Extraire le mapper dans un fichier séparé |
| Fichier de config (`.ts`) | **50 lignes** | Diviser par fonctionnalité |
| Fichier de tests (`.test.ts`) | **100 lignes** | Diviser par `describe` dans des fichiers séparés |

### Stratégies de Découpage

```typescript
// ❌ MAUVAIS : Un seul fichier de 200 lignes
// StockPage.tsx — 200 lignes !!!

// ✅ BON : Découper en fichiers distincts < 80 lignes chacun
// StockPage.tsx          (50 lignes) — Assemblage des sections
...
// ❌ MAUVAIS : Un hook useStock.ts de 180 lignes

// ✅ BON : Découper par responsabilité
...
```json
// .eslintrc.json — Ajouter cette règle pour bloquer les fichiers trop longs
{
  "rules": {
    "max-lines": ["error", {
      "max": 100,
...
[ ] Aucun fichier du projet ne dépasse 100 lignes (règle absolue)
