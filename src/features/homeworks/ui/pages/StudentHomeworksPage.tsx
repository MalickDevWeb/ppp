    ...
    <section className="max-w-[1280px] mx-auto px-4 md:px-8 py-6 md:py-8 animate-fade-in relative">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl border border-neutral-gray-200/50">
      ...
      <div className="bg-white border border-neutral-gray-200 rounded-3xl p-4 mb-8 flex flex-col md:flex-row items-stretch md:items-center gap-4 shadow-3xs">
        ...
        <div className="flex-grow min-w-[200px] relative">
          <span translate="no" className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-gray-500 text-[18px]">filter_list</span>
          <select 
            value={selectedCours}
            onChange={(e) => {
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-[#FFF8F7]/40 rounded-xl focus:outline-none focus:bg-[#FFF8F7]/80 focus:ring-1 focus:ring-brand-red-deep/20 text-xs font-black text-[#291715] appearance-none cursor-pointer outline-none border border-neutral-gray-200"
          >
          ...
          <span translate="no" className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-neutral-gray-400 pointer-events-none text-[16px]">expand_more</span>
        </div>

        <div className="h-6 w-px bg-neutral-gray-200 hidden md:block" />

        {/* Priority Filter is beautifully scrollable on mobile! */}
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar max-w-full flex-nowrap shrink-0 -mx-4 px-4 md:mx-0 md:px-0">
          <span className="text-[10px] font-extrabold uppercase tracking-wider text-secondary whitespace-nowrap shrink-0">Priorité :</span>
          <div className="flex gap-1.5 flex-nowrap select-none">
            {['toutes', 'haute', 'normale'].map((prio) => (
              <button
                key={prio}
                onClick={() => {
                  setSelectedPriority(prio);
                  triggerToast(`Priorité filtrée : ${prio === 'toutes' ? 'Toutes' : prio}`);
                }}
                className={`px-3.5 py-2.5 rounded-full font-bold text-xs transition-all cursor-pointer whitespace-nowrap shrink-0 snap-start ${
                  selectedPriority === prio
                    ? 'bg-brand-red-deep text-white shadow-xs'
                    : 'bg-neutral-gray-100 text-secondary hover:bg-neutral-gray-200'
                }`}
              >
                {prio === 'toutes' ? 'Toutes' : prio === 'haute' ? '🔴 Haute' : '🔵 Normale'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 ml-auto shrink-0 justify-between">
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="flex items-center gap-1 text-xs font-black text-[#291715] hover:text-brand-red-deep transition-colors cursor-pointer bg-neutral-gray-50 px-3 py-1.5 rounded-lg"
            >
              Échéance
              <span translate="no" className="material-symbols-outlined text-[14px]">expand_more</span>
            </button>
          </div>
        </div>
