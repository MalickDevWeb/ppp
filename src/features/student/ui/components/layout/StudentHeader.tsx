...
export function StudentHeader({
  notifications, announcements, unreadCount, notifFilter, setNotifFilter,
  ...
  return (
    <header className="w-full top-0 sticky z-[100] bg-white/90 backdrop-blur-md border-b border-neutral-gray-200/80 shadow-sm animate-fade-in">
      {(showProfile || showNotifications || showCalendar) && (
        <div onClick={() => { setShowProfile(false); setShowNotifications(false); setShowCalendar(false); }} className="fixed inset-0 bg-[#291715]/15 backdrop-blur-xs z-[180] cursor-pointer" />
      )}
      <div className="flex items-center justify-between px-6 md:px-8 w-full h-16 gap-4 relative z-[190]">
        ...
        <div className="relative hidden sm:block cursor-pointer group flex-grow max-w-3xl">
          <div className="relative pl-10 pr-12 py-2.5 bg-neutral-gray-50/95 border border-neutral-gray-200/80 rounded-2xl text-xs font-semibold overflow-hidden transition-all duration-300 w-full flex items-center justify-between shadow-3xs group-hover:border-brand-red-deep/45 group-hover:bg-white group-hover:shadow-sm">
            <span translate="no" className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-gray-400 group-hover:text-[#B3181C] transition-colors text-[18px]">search</span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="relative" ref={calendarPopRef}>
            ...
            {showCalendar && (
              <div className="fixed top-16 left-4 right-4 sm:absolute sm:top-full sm:left-auto sm:right-0 sm:mt-3 w-[calc(100vw-32px)] sm:w-[650px] md:w-[800px] bg-white border border-neutral-gray-250 rounded-3xl shadow-2xl p-5 space-y-4 max-h-[85vh] overflow-y-auto z-[200] animate-slide-up no-scrollbar">
                <div className="flex items-center justify-between pb-3 border-b border-neutral-200 select-none">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-title-lg text-xs font-black uppercase text-[#291715] tracking-wide">Calendrier des Cours & Événements</h4>
