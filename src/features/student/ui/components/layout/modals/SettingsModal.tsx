...
export function SettingsModal({ showSettings, setShowSettings, settingsPrefs, setSettingsPrefs, triggerToast }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[260] flex items-center justify-center p-4 animate-fade-in select-none">
      <div className="bg-white rounded-3xl w-full max-w-sm border border-neutral-gray-200/50 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-gradient-to-br from-brand-red-deep to-[#7a1012] p-5 text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2"><span className="material-symbols-outlined text-base">settings</span><h4 className="font-extrabold text-xs">Options & Coeur du Système</h4></div>
        ...
        <div className="p-5 space-y-4 flex-grow overflow-hidden">
          <span className="block text-[9px] font-black uppercase tracking-widest text-[#B3181C] mb-1">Paramètres Personnalisés</span>
          {[
            ...
            <div key={key} className="flex items-center justify-between gap-4">
              <div><h5 className="text-xs font-bold text-[#291715]">{label}</h5><p className="text-[10px] text-neutral-gray-400">{desc}</p></div>
          ...
          <div className="space-y-1.5 pt-1">
            <label className="block text-[10.5px] font-extrabold text-[#291715]">Langue de navigation</label>
        ...
        <div className="p-4 bg-neutral-gray-50 border-t border-neutral-gray-150 flex gap-2 shrink-0">
          <button onClick={() => { setShowSettings(false); triggerToast('Préférences synchronisées', 'save_as'); }} className="flex-1 py-2.5 bg-brand-red-deep hover:bg-[#8F1316] text-white rounded-xl text-xs font-bold transition-all shadow-md cursor-pointer">Sauvegarder</button>
