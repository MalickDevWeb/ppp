...
export function DocModal({ showDocModal, setShowDocModal, openDocSection, setOpenDocSection }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[260] flex items-center justify-center p-4 animate-fade-in select-none">
      <div className="bg-white rounded-3xl w-full max-w-md border border-neutral-gray-200/50 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex-grow p-4 space-y-3 overflow-y-auto no-scrollbar max-h-[55vh]">
            ...
            {DOC_SECTIONS.map((sec, idx) => {
              ...
              return (
                <div key={idx} className="border border-neutral-gray-250 rounded-xl overflow-hidden bg-neutral-gray-50/50">
                  <button type="button" onClick={() => setOpenDocSection(isOpen ? null : idx)} className="w-full text-left p-3 flex justify-between items-center bg-white hover:bg-neutral-gray-50 font-bold text-xs text-[#291715] select-none cursor-pointer">
                    {sec.title}
                    <span className="material-symbols-outlined text-xs text-neutral-gray-400">{isOpen ? 'expand_less' : 'expand_more'}</span>
                  </button>
                  {isOpen && <div className="p-3 border-t border-neutral-200 text-[10.5px] text-[#291715] leading-relaxed bg-white">{sec.content}</div>}
                </div>
              );
            })}
          </div>
        ...
        <div className="p-4 bg-neutral-gray-50 border-t border-neutral-gray-150 shrink-0 text-right">
          <button onClick={() => setShowDocModal(false)} className="w-full py-2.5 bg-brand-red-deep text-white hover:bg-[#8F1316] rounded-xl text-xs font-bold shadow-sm transition-all cursor-pointer">Fermer</button>
        </div>
      </div>
    </div>
  );
