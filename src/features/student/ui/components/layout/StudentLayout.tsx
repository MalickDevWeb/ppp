...
export function StudentLayout() {
  ...
  const [settingsPrefs, setSettingsPrefs] = useState({ emailNotifs: true, proactiveAiTutor: true, smoothScroll: true, language: 'Français', experimentalDark: false });
  ...
  const handleAiTutor = () => {
    ...
    setTimeout(() => { const el = document.getElementById('ai-tutor-assistant'); if (el) { el.scrollIntoView({ behavior: 'smooth', block: 'center' }); el.classList.add('ring-4', 'ring-brand-red-deep/40'); setTimeout(() => el.classList.remove('ring-4', 'ring-brand-red-deep/40'), 2500); } }, 300);
    triggerLayoutToast('Redirection vers le Tuteur IA');
  };
  ...
  return (
    <div className="bg-surface text-on-surface font-body-md min-h-screen flex selection:bg-brand-red-light selection:text-brand-red-deep">
      ...
      {/* Layout Toast */}
      {toastMessage && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-100 bg-neutral-gray-900 border border-white/10 text-white rounded-2xl py-3.5 px-6 shadow-2xl flex items-center gap-3 animate-fade-in text-sm font-bold min-w-[320px] max-w-[500px]">
      ...
      <StudentSidebar
        ...
        onOpenAiTutor={handleAiTutor}
        triggerToast={triggerLayoutToast}
      />

      {/* Main content */}
      <div className="flex-1 md:ml-20 lg:ml-64 bg-background min-h-screen pb-20 md:pb-0 flex flex-col min-w-0 max-w-full overflow-x-hidden">
        <StudentHeader
      ...
      <SettingsModal
        ...
        settingsPrefs={settingsPrefs as any} setSettingsPrefs={setSettingsPrefs as any}
        triggerToast={triggerLayoutToast}
      />
      <SupportModal
